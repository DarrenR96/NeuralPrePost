...
class ExtractNumpyFlow(FlowSpec):
    ...
    def start(self):
        ...
        train_test_values_df.to_csv(os.path.join(self.output_folder, 'video_split.csv'), index=False)
        self.videos_to_extract = train_test_values_df.to_dict(orient='records')
    ...
    def extract_patches(self):
        ...
        self.written_data = []
        for frame_num, _reference_frame in enumerate(reference_video):
            ...
            for (_ref_hr_y, _ref_hr_uv) in zip(_reference_frame_y_blocks, _reference_uv_blocks):
                out_filename = str(uuid.uuid4()) + '.npz'
                out_filepath = os.path.join(output_folder, out_filename)
                np.savez(out_filepath, reference_hr_y=_ref_hr_y, reference_hr_uv=_ref_hr_uv)
